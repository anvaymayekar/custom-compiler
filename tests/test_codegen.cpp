...
int compileAndRun(const std::string &src, const std::string &tag) {
    ...
    if (std::system(("nasm -f elf64 " + asmPath + " -o " + objPath).c_str()) != 0) {
...
MT_TEST(assembly_contains_no_forbidden_8bit_multiply) {
    ...
    // values" bug: the backend must multiply via the full 64-bit register
    // (`mul rbx`), never a narrower al/ax/eax form.
    MT_CHECK(asmText.find("mul rbx") != std::string::npos);
...
MT_TEST(parentheses_change_the_result_vs_default_precedence) {
    ...
    const int withoutParens = compileAndRun("shevti(10 + 90 * 5);\n", "noparen");
    const int withParens = compileAndRun("shevti((10 + 90) * 5);\n", "withparen");
...
MT_TEST(nahitar_branch_selected_when_first_condition_false) {
    ...
    const int code = compileAndRun(
        "ank a = 0;\n"
        "ank b = 1;\n"
        "jar (a) { shevti(1); } nahitar (b) { shevti(7); } anyatha { shevti(9); }\n",
        "nahitar");
    MT_CHECK_EQ(code, 7);
}
